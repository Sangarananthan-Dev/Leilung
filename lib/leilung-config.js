...
export const DEPARTMENT_CONFIG = {
  food: {
    ...
    layerOptions: [
      { key: "pdsCoverage", label: "Public Distribution System coverage" },
      { key: "fpsShops", label: "Ration shop markers" },
    ...
    getTooltipMetric(district) {
      ...
      return `${titleCase(district.smartPdsStatus)} Smart Public Distribution System · ${formatNumber(district.undistributedStockQtl)} qtl pending`;
    },
    buildKpis(dataset) {
      return [
        {
    ...
    buildDistrictView(district) {
      return {
        summary: `${district.totalBeneficiaries.toLocaleString("en-IN")} beneficiaries across ${district.fpsShops} ration shops.`,
        status: {
          label: `Smart Public Distribution System ${titleCase(district.smartPdsStatus)}`,
          tone:
        ...
        facts: [
          {
            label: "Digital ration device enabled",
