...
export const STATEWIDE_ALERTS = [
  ...
  "✓ e-PoS authentication successful — 98% completion in Kolasib this cycle",
...
const LAND_CASE_DETAILS = {
  "lawngtlai:ENC-LWT-0044": {
    ...
    encroacher: "[Name redacted for demo] — 3 structures built",
    applicableAct:
  ...
  return {
    ...
    kpis: [
      ...
      },
      {
        ...
        value: "692",
        trend: "▼ down from 1,240 (post e-PoS)",
        href: "#ghost-report",
    ...
    ePosDonut: [
      { label: "e-PoS active", value: 1056, color: "#22c55e" },
    ...
    alerts: STATEWIDE_ALERTS.filter((item) => {
      return (
        item.includes("PDS") || item.includes("e-PoS") || item.includes("Ghost")
      );
    }),
  };
}
...
export function getFoodDistrictData(districtId) {
  ...
  return {
    ...
    headerStats: [
      ...
      `${district.fpsShops} FPS shops (${district.fpsEposEnabled} e-PoS active, ${district.fpsShops - district.fpsEposEnabled} offline)`,
...
export function getFoodShopData(districtId, shopId) {
  ...
  return {
    ...
    beneficiariesServed: `${Math.round(shop.distributionPercent * 12)} active beneficiary households (simulated record)`,
    operatingSince: "2014",
...
export function getLandCaseData(districtId, caseId) {
  ...
  return {
    ...
    encroacher: "Operational record (simulated)",
